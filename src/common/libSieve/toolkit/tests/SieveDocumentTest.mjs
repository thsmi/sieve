...
suite.add("Get spec by invalid name", () => {
  ...
  suite.assertThrows(
    ...
    "Invalid node name @action/something.");
});
...
suite.add("Element by unknown name", () => {
  ...
  suite.assertThrows(
    () => { doc.probeByName("action/something", "document"); },
    "No specification for >>action/something<< found");
...
suite.add("Element by invalid type", () => {
  ...
  suite.assertThrows(
    ...
    "Invalid type name action/something");
...
suite.add("Element by unknown type", () => {
  ...
  suite.assertThrows(
    () => { doc.createByClass("@action/something", "document"); },
    "Unknown or incompatible type >>@action/something<< at >>document<<");
});
