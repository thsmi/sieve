...
function SieveImapAuth(imapKey) {
  if (!imapKey)
    throw new Error("SieveImapAuth: IMAP account key can't be null");
...
SieveImapAuth.prototype.getPassword
  = function () {
    ...
    let account = Cc['@mozilla.org/messenger/account-manager;1']
...
function SieveCustomAuth2(host, uri) {
  if (!uri)
    throw new Error("SieveCustomAuth2: URI can't be null");

  if (!host || host === "")
    throw new Error("SieveCustomAuth2: Host can't be empty or null");
...
/**
 * Uses the proxy configured by the operating system
 * @constructor
 */
...
/**
 * Uses an socks v4 proxy
 * @param {String} sieveKey
 *   the sieve account's unique id
 * @constructor
 */
...
function SieveAbstractHost(uri) {
  if (!uri)
    throw new Error("SieveCustomHost: URI can't be null");
...
function SieveImapHost(uri, imapKey) {
  ...
  if (!imapKey)
    throw new Error("SieveImapHost: IMAP account key can't be null");
...
SieveCustomHost.prototype.setHostname
  = function (hostname) {
    if (hostname == null)
      throw new Error("Hostname can't be null");
...
function SieveCustomAuthorization(uri) {
  if (!uri)
    throw new Error("SieveCustomAuthorization: URI can't be null");
...
SieveAccount.prototype.isFirstRun
  = function () {
    ...
    if (this.getHost().getType() === HOST_TYPE_CUSTOM) {
      this.setFirstRun();
      return false;
    }
...
SieveAccount.prototype.getProxy
  = function (type) {
    ...
    switch (type) {
      case PROXY_TYPE_NONE: return new SieveNoProxy();
      case PROXY_TYPE_SOCKS4: return new SieveSocks4Proxy(this.sieveKey);
      case PROXY_TYPE_SOCKS5: return new SieveSocks5Proxy(this.sieveKey);
...
const SieveAccountManager = new SieveAccounts();
